...
function missingRequiredFields(candidate) {
  return REQUIRED_FIELDS.filter((field) => candidate[field] === undefined || candidate[field] === null);
}
...
function findUnsupportedTechnicalValues(candidate) {
  if (isMissing(candidate.source_page) || isMissing(candidate.raw_row_ref)) {
    return TECHNICAL_FIELDS.filter((field) => hasValue(candidate[field]));
  }
  return [];
}
...
function buildCandidateValidation(candidate, options = {}) {
  ...
  const unsupportedTechnicalValues = findUnsupportedTechnicalValues(normalized);
  if (unsupportedTechnicalValues.length) {
    addFlag(normalized.risk_flags, 'UNSUPPORTED_TECHNICAL_VALUE');
    errors.push('UNSUPPORTED_TECHNICAL_VALUE');
  }
